...
## 2026-08-06: Multi-provider profiles, per profile models (Step 12)

- The single per-user `user_settings` row was too rigid: one provider config per user meant switching gateways required editing settings each time. It is replaced by `llm_providers` (named profiles: name, `openai` type, base URL, API key, `is_default`), each with `llm_models` rows (name, `is_default`), so a user keeps several gateways (OpenRouter, Ollama, a local proxy...) and a model list per gateway. Models are rows, not a JSON blob, for the same reason tool calls are rows: queryable, replaceable per profile, and sqlc-friendly. The Step 11 table was dropped in 00007 (pre-launch, and the goose Down restores it).
- Defaults are explicit flags, not order magic: the first profile a user creates becomes the default, `default: true` on create/update moves it (clearing the others), and `default: false` on update deliberately leaves the flag alone so an ordinary edit can never silently drop the user's default. Same for `default_model` within a profile. A profile with no models at all is legal (ad-hoc model overrides still work) but resolution without one fails early with a clear message instead of hitting the gateway.
- Resolution precedence per run: explicit `provider_id` on the chat request > the user's default profile > the server default provider from env config; the request's `model` overrides the profile's default model. `chat.Resolver.Resolve` gained `providerID` and `model` parameters and `Engine.Run` passes them through; the scheduler and approval resume pass 0 (the user's default), so jobs keep running with whatever the user currently defaults to. Model is baked into the provider when the request doesn't carry one, and the OpenAI adapter's `req.Model`-wins behavior keeps the override path consistent.
- API keys stay plaintext (the Step 11 note stands) and the same boundary rule holds: the providers API never returns a key, only `has_api_key`, and an empty `api_key` on update keeps the stored one. `provider` is validated to `openai` because that protocol covers every OpenAI-compatible gateway; the field exists so other gateway types can join the enum later.
- The chat composer's free-text model override is now a picker: optgroups per provider, one option per model, "Default provider & model" on top, so switching gateways mid-conversation is a dropdown choice. The Settings page became a provider manager (add/edit/delete, models as a comma list, default flags, per-profile key retention).
- Deferred: per-job provider selection (jobs use the owner's default), provider-side model listing/validation against the gateway (typos surface as provider errors at run time), and auto-promotion when the default profile is deleted (no default then means fall back to the server default).
